...
sysName = 'TF-System#6_1R' #System Name here
initCapital = 500000
commission = 50
#----------------------------------------------------------------------------------
...
#Additiona Imports can go below here
import operator
...
#---------------------------------------------------------------------------------------------------
            ...
            if isNewMonth(masterDateList,curPortBar) and firstMarketLoop == False and curMarket == 0:
                tempList = adxMarketValList[-numMarkets:]
                tempList.sort(key=operator.itemgetter(1),reverse=True)
                for mkt in range(0,numMarkets):
                    stopTrading[mkt] = True
#               print(masterDateList[curPortBar])
                numMarksToTurnOff = min(numMarkets,15)
                for turnOn in range(0,numMarksToTurnOff):
                    stopTrading[tempList[turnOn][0]] = False
...
#---------------------------------------------------------------------------------------------------
            ...
            myADX,myADXR,myDMI =adxList[curMarket].calcADX(myHigh,myLow,myClose,20,curBar,0)
            adxMarketValList.append((curMarket,myADX))
...
#  Okay  Let's put in some logic to create a long position
            if  myHigh[curBar] >= buyLevel and mp !=1 and stopTrading[curMarket] == False:
...
#  Okay  Let's put in some logic to create a short position
            if myLow[curBar] <= shortLevel and mp !=-1 and stopTrading[curMarket] == False:
