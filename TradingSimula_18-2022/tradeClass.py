...
##            totEntryQuant += entryQuant[numEntries]
        ...
        tempNumShares = numShares
#        if numEntriesLookBack == 1: peelAmt = tempNumShares
        for numEntries in range(0,numEntriesLookBack):
            ...
            if order == 'buy':
                if curPos < 0:
                    ...
                    profitPerEntry.append((entryPrice[0] - exitPrice) * peelAmt)
            elif order == 'sell':
                if curPos > 0:
                    profit = profit + (exitPrice - entryPrice[0]) * peelAmt
                    profitPerEntry.append((exitPrice - entryPrice[0]) * peelAmt)
            elif order == 'liqLong':
                if curPos > 0:
                    profit = profit + (exitPrice - entryPrice[0]) * peelAmt
                    profitPerEntry.append((exitPrice - entryPrice[0]) * peelAmt)
