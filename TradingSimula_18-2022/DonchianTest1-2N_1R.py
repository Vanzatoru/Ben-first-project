...
sysName = 'Donch-Test-2N_1R' #System Name here
initCapital = 100000
commission = 100
#----------------------------------------------------------------------------------
...
#  Define Long, Short, ExitLong and ExitShort Levels - mind your indentations
            ...
            longExit = lowest(myLow,15,curBar,1)
            shortExit = highest(myHigh,15,curBar,1)
            ATR = sAverage(myTrueRange,20,curBar,1)
...
#  Okay  Let's put in some logic to create a long position
 ...
 # Long Exit
            ...
            longExit > marketVal1[curMarket] and barsSinceEntry > 1:
                ...
                unPackDict(sysMarkDict)
 # Long Exit
            if mp == 1 and myLow[curBar] <= marketVal1[curMarket] and barsSinceEntry > 1:
                price = min(myOpen[curBar],marketVal1[curMarket])
                tradeName = "Lxit2N"
                numShares = curShares
                exitPosition(price, curShares, tradeName, sysMarkDict)
                unPackDict(sysMarkDict)
...
#  Short Exit
            ...
            shortExit < marketVal1[curMarket] and  barsSinceEntry > 1:
            ...
            if mp == -1 and myHigh[curBar] >= marketVal1[curMarket] and barsSinceEntry > 1:
                price = max(myOpen[curBar],marketVal1[curMarket])
                tradeName = "Sxit2N"
                numShares = curShares
                exitPosition(price, curShares, tradeName,sysMarkDict)
                unPackDict(sysMarkDict)
