...
#-----------
            ...
            upBand2,dnBand2, midBand2 = bollingerBands(myDate,myClose,80,1.75,curBar,1)
            cm = curMarket

            if mp != 1 and marketVal1[cm] != 1 and myClose[curBar-1] > upBand1:
                marketVal1[cm] = 1
                marketVal2[cm] = upBand2
            if mp !=-1 and marketVal1[cm] != -1 and myClose[curBar-1] < dnBand1:
                marketVal1[cm] = -1
                marketVal2[cm] = dnBand2

            if marketVal1[cm] == 1 and myClose[curBar-1] <= midBand1: marketVal1[cm] = 0
            if marketVal1[cm] ==-1 and myClose[curBar-1] >= midBand1: marketVal1[cm] = 0
...
#  Okay  Let's put in some logic to create a long position
            if myClose[curBar-1] < marketVal2[cm] and marketVal1[cm] == 1 and \
            myClose[curBar-1] > midBand1 and mp !=1:
                marketVal1[cm] = 0
                price = myOpen[curBar]
