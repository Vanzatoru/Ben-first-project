...
sysName = 'TF-BollOnClose_1R' #System Name here
initCapital = 500000
commission = 50
#----------------------------------------------------------------------------------
...
#  Define Long, Short, ExitLong and ExitShort Levels - mind your indentations
            upBand,dnBand, midBand = bollingerBands(myDate,myClose,80,2,curBar,0)
...
#  Okay  Let's put in some logic to create a long position
            if myClose[curBar] >= buyLevel and mp !=1:
...
#  Long Exit
            if mp == 1 and myClose[curBar] <= longExit and barsSinceEntry > 1:
                price = myClose[curBar]
                tradeName = "Lxit"
                numShares = curShares
...
#  Okay  Let's put in some logic to create a short position
            if myClose[curBar] <= shortLevel and mp !=-1:
                price = myClose[curBar]
                ...
                unPackDict(sysMarkDict)
#  Short Exit
            if mp == -1 and myClose[curBar] >= shortExit and barsSinceEntry > 1:
                price = myClose[curBar]
                tradeName = "Sxit"
                numShares = curShares
                exitPosition(price, curShares, tradeName,sysMarkDict)
