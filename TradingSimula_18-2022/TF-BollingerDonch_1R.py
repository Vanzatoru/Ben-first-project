...
sysName = 'TF-Boll.Donch_1R' #System Name here
initCapital = 100000
commission = 50
#----------------------------------------------------------------------------------
...
#-----------
            ...
            donchHi = highest(myHigh,40,curBar,1)
            donchLo = lowest(myLow,40,curBar,1)

            longEntry = max(upBand1,donchHi)
            shortEntry = min(dnBand1,donchLo)
            ...
            risk = abs(upBand1 - midBand1)*myBPV
            posSize = 1

#  Long Entry
#  Okay  Let's put in some logic to create a long position
            if myHigh[curBar] >= longEntry and mp !=1:
                price = max(myOpen[curBar],longEntry)
...
#  Okay  Let's put in some logic to create a short position
            if myLow[curBar] <= shortEntry and mp !=-1:
                price = min(myOpen[curBar],shortEntry)
