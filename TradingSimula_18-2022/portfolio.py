...
class portfolioClass(object):
    ...
    def setPortfolioInfo(self,name,systemMarket):
        ...
        if name == "PortfolioTest":
            printMonthlyToTerminal = True
#        exportToXL = True
        ...
        if name == "PortfolioTest":
            ...
            exportToXL = True
        if name == "OptimizeTest":
#            if os.path.exists(self.systemMarkets[0].systemName + "-Optimize.txt"):
#                os.remove(self.systemMarkets[0].systemName + "-Optimize.txt")
            fileName1 = self.systemMarkets[0].systemName + "-Optimize.txt"
...
#            wb.save(xlFileName)
        ...
        for i in range(0,len(self.systemMarkets)):
            ...
            if i == 0:
                if self.isOptimize:
                    lineOutPut = str(list(self.optSysParams.keys()))
                ...
                if name == "OptimizeTest":
                    lineOutPut = 'Testing from : ' + str(begDate) + ' to: ' + str(endDate) + ',\n'
                    target1.write(lineOutPut)
                    lineOutPut = 'Parameters,Market,TotProfit,MaxDD,ClsTrdDD,AvgWin,AvgLoss,PerWins,#Trds,MonthRet.,StdDev,YearlyRet.\n'
                    target1.write(lineOutPut)
            ...
            if name == "OptimizeTest":
                lineOutPut = str(list(self.optSysParams.items()))
                lineOutPut = lineOutPut + ","+comName+"-"+market+","+str(totProf)+","+str(maxD)+","+str(clsTrdDD)+","+str(avgWin)+","+str(avgLoss)+","+str(perWins)+","+str(totTrades)+","+str(avgMonthlyRet)+","+str(monthlyStdDev)+","+str(avgYearlyRet)+'\n'
                target1.write(lineOutPut)
        print('-------------------------------------------------------------------------------------------------------------------')
        lineOutPut = '-------------------------------------------------------------------------------------------------------------------\n'
        target1.write(lineOutPut)
...
#                print(masterDateList[i]," ",boyDrawDown)
 ...
 #           print(cumuVal," ",self.portPeakEquity," ",cumuVal - self.portPeakEquity)
            if name == "PortfolioTest": target3.write('%8d %8.2f\n' % (masterDateList[i],cumuVal - self.portPeakEquity))
...
##            chartObj.set_categories(dates)
        ...
        if len(monthEquity) != 0: mean = sum(monthEquity) / len(monthEquity)
        ...
        if name == "PortfolioTest": target1.write('Average Monthly Return: %8d StdDev: %5d  Sharpe: %4.2f\n' % (mean,sd,mean/sd))
        ...
        if name == "PortfolioTest": target1.write('Average  Yearly Return: %8d StdDev: %5d  Sharpe: %4.2f\n' % (mean,sd,mean/sd))
        target1.close()
        if printOutDailyEquity: target2.close()
        if name == "PortfolioTest": target3.close()
