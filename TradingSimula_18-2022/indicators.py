...
class laguerreRSIClass(object):
    def __init__(self):
        ...
        self.LaguerreRSI = 0
        self.seed = 0
        self.L0.extend((0,0,0))
        self.L1.extend((0,0,0))
        self.L2.extend((0,0,0))
        self.L3.extend((0,0,0))


    def calcLaguerreRSI(self,cPrices,gamma,curBar,offset):
        self.seed += 1
        self.L0.append((1 - gamma)*cPrices[curBar-offset] + gamma*self.L0[-1])
        if self.seed > 1:
            self.L1.append( -gamma * self.L0[-1] + self.L0[-2] + gamma*self.L1[-1])
        if self.seed > 2:
            self.L2.append( -gamma * self.L1[-1] + self.L1[-2] + gamma*self.L2[-1])
        if self.seed > 3:
            self.L3.append( -gamma * self.L2[-1] + self.L2[-2] + gamma*self.L3[-1])
        if self.seed > 3:
            self.CU = 0
            self.CD = 0
            if self.L0[-1] >= self.L1[-1]:
                self.CU = self.L0[-1] - self.L1[-1]
            else:
                self.CD = self.L1[-1] - self.L0[-1]

            if self.L1[-1] >= self.L2[-1]:
                self.CU = self.CU + self.L1[-1] - self.L2[-1]
            else:
                self.CD = self.CD + self.L2[-1] - self.L1[-1]

            if self.L2[-1] >= self.L3[-1]:
                self.CU = self.CU + self.L2[-1] - self.L3[-1]
            else:
                self.CD = self.CD + self.L3[-1] - self.L2[-1]
            if self.CU + self.CD != 0:
                self.LaguerreRSI = self.CU / (self.CU + self.CD)
            else:
                self.LaguerreRSI = 0
        return(self.LaguerreRSI)
